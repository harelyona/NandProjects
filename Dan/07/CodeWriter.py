...
class CodeWriter:
    ...
    def __write_equal_command(self) ->None:
        ...
        self.output_stream.write(f"@NOT_EQUAL{self.bool_num}\n")
    ...
    def __bool_comparison_command(self, command: str) -> None:
        ...
        # Else we need to handle the gt and lt case:
            ...
            self.output_stream.write("AM=M-1\n")
            self.output_stream.write("D=M\n")
            self.output_stream.write(f"@Y_NEGATIVE{self.bool_num}\n")
            ...
            self.output_stream.write("A=M-1\n")
            self.output_stream.write("D=M\n")
            self.output_stream.write(f"@X_NEGATIVE_Y_POSITIVE{self.bool_num}\n")
            self.output_stream.write(f"D;JLT\n") # Jump if x<0
            ...
            self.output_stream.write(f"@END_BOOL{self.bool_num}\n")
            if command == "gt":
            ...
            self.output_stream.write("A=M-1\n")
            self.output_stream.write("D=M\n")
            self.output_stream.write(f"@X_AND_Y_NEGATIVE{self.bool_num}\n")
            self.output_stream.write(f"D;JLT\n") # Jump if x<0
